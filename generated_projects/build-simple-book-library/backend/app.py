...
logger = logging.getLogger("Build Simple Book Library")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "build_simple_book_library.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Build Simple Book Library Backend",
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Build Simple Book Library Backend on http://{host}:{port}")
