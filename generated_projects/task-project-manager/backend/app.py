...
logger = logging.getLogger("Task and Project Manager")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "task_and_project_manager.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Task and Project Manager Backend",
    ...
    def get_projects():
        try:
            ...
            if search:
                from sqlalchemy import or_
                query = query.filter(or_(Project.title.ilike(f'%{search}%')))
    ...
    def create_project():
        try:
            ...
            db.session.add(item)
            db.session.commit()
            return jsonify({"success": True, "message": "Project created successfully", "data": item.to_dict()}), 201
        except Exception as e:
            db.session.rollback()
    ...
    def update_project(item_id):
        try:
            ...
            db.session.commit()
            return jsonify({"success": True, "message": "Project updated successfully", "data": item.to_dict()}), 200
        except Exception as e:
            db.session.rollback()
    ...
    def get_tasks():
        try:
            ...
            if search:
                from sqlalchemy import or_
                query = query.filter(or_(Task.title.ilike(f'%{search}%')))
    ...
    def create_task():
        try:
            data = request.get_json() or {}
            item = Task()
            if 'project_id' in data: item.project_id = int(data['project_id'])
            if 'title' in data: item.title = str(data['title']).strip()
            if 'description' in data: item.description = str(data['description']).strip()
            if 'status' in data: item.status = str(data['status']).strip()
            if 'priority' in data: item.priority = str(data['priority']).strip()
            if 'due_date' in data and data['due_date']:
                try: item.due_date = datetime.fromisoformat(data['due_date'])
                except Exception: pass
            if 'assigned_to' in data: item.assigned_to = str(data['assigned_to']).strip()

            db.session.add(item)
            db.session.commit()
            return jsonify({"success": True, "message": "Task created successfully", "data": item.to_dict()}), 201
        except Exception as e:
            db.session.rollback()
    ...
    def update_task(item_id):
        try:
            ...
            db.session.commit()
            return jsonify({"success": True, "message": "Task updated successfully", "data": item.to_dict()}), 200
        except Exception as e:
            db.session.rollback()
...
def _seed_initial_data():
    ...
    try:
        ...
        if Project.query.count() == 0:
            # Seed demo Project records
            db.session.add(Project(title='Sample Project Title 1', description='Initial demo record 1 populated for system testing.', status='Active', created_at='Sample created_at 1'))
            db.session.add(Project(title='Sample Project Title 2', description='Initial demo record 2 populated for system testing.', status='Active', created_at='Sample created_at 2'))
            db.session.commit()
        if Task.query.count() == 0:
            # Seed demo Task records
            db.session.add(Task(project_id=10, title='Sample Task Title 1', description='Initial demo record 1 populated for system testing.', status='Active', priority='Sample priority 1', due_date='Sample due_date 1', assigned_to='Sample assigned_to 1'))
            db.session.add(Task(project_id=20, title='Sample Task Title 2', description='Initial demo record 2 populated for system testing.', status='Active', priority='Sample priority 2', due_date='Sample due_date 2', assigned_to='Sample assigned_to 2'))
            db.session.commit()
    except Exception as e:
        db.session.rollback()
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Task and Project Manager Backend on http://{host}:{port}")
