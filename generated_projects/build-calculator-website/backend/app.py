...
logger = logging.getLogger("Build Calculator Website")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "build_calculator_website.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Build Calculator Website Backend",
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Build Calculator Website Backend on http://{host}:{port}")
