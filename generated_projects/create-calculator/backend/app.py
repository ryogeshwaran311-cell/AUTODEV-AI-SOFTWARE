...
logger = logging.getLogger("Create Calculator")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "create_calculator.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Create Calculator Backend",
    ...
    def serve_frontend(path):
        ...
        return jsonify({
            "service": "Create Calculator Full-Stack Application",
            "status": "Running (Development/API mode)",
            "message": "Frontend static bundle not compiled yet. Run 'npm run build' inside frontend/ for static UI bundle."
        }), 200
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Create Calculator Backend on http://{host}:{port}")
