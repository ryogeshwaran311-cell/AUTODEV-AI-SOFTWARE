...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "simple_calculator.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Simple Calculator Backend",
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Simple Calculator Backend on http://{host}:{port}")
