...
logger = logging.getLogger("Build Simple Crm With")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "build_simple_crm_with.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Build Simple Crm With Backend",
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Build Simple Crm With Backend on http://{host}:{port}")
