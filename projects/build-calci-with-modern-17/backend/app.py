...
logger = logging.getLogger("Build Calci With Modern")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "build_calci_with_modern.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Build Calci With Modern Backend",
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Build Calci With Modern Backend on http://{host}:{port}")
