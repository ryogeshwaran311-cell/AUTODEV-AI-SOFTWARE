...
logger = logging.getLogger("Build Library Management System")
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', f'sqlite:///{os.path.join(base_dir, "build_library_management_system.db")}')
    ...
    def health_check():
        return jsonify({
            "status": "healthy",
            "service": "Build Library Management System Backend",
...
if __name__ == '__main__':
    ...
    app = create_app()
    logger.info(f"Starting Build Library Management System Backend on http://{host}:{port}")
